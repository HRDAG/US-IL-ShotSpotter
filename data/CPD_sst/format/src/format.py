...
def format_datetime_partial(x):
    if pd.isna(x): return None
    out = pd.to_datetime(x, format="%d-%b-%Y %H:%M")
    return out
...
def format_sst(raw):
    ...
    for col in datecols:
        ...
        copy[col] = copy[col].apply(format_datetime_partial)
...
if __name__ == '__main__':
    ...
    cpd_sst = format_sst(raw=cpd_sst)
    assert cpd_sst.loc[cpd_sst.duplicate_event.isna(), 'year_called'].isna().all(), f"\
        `duplicate_event` should only be NA for the OEMC records."
